...
*******************************************************************/
bool SerialInterface::openSyn(wstring serial_name, unsigned int baud_rate, unsigned char parity, unsigned char byte_size, unsigned char stop_bits)
{
...
* 返回： 正确返回为ture，错误返回为false
*******************************************************************/
bool SerialInterface::openSyn(wstring serial_name)
{
    hCom = CreateFileW(
    ...
    {
        hCom = NULL;
        setSerialLastError("SerialInterface::openSyn():CreateFileW Error!");
        cout << last_error << endl;
        return false;
    }
    return true;
...
DWORD SerialInterface::readData(char *buffer, int length)
{
    ...
    ret = ReadFile(hCom, buffer, length, &bytesRead, NULL);

    if (false == ret)
    {
        setSerialLastError("SerialInterface::readData  Error");
        cout << last_error << endl;
        return 0;
    }
...
DWORD SerialInterface::readCurrentData(char *buffer, int length)
{

    if (false == PurgeComm(hCom, PURGE_RXCLEAR | PURGE_RXABORT))
    {
        setSerialLastError("SerialInterface::readCurrentData() : PurgeComm Error");
        cout << last_error << endl;
        return false;
    }
    ...
    if (false == ret)
    {
        setSerialLastError("SerialInterface::readCurrentData  Read Error");
        cout << last_error << endl;
        return 0;
    }
